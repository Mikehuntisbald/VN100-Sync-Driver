...
bool ImuVn100::SyncInfo::Update(const unsigned sync_count,
  ...
  if (count != sync_count) {
    ...
    count = sync_count;
    time = ros::Time().fromSec(std::floor(sync_time.toSec()));
    cv_.notify_all();
    ret = true;
  }
  return ret;
...
void ImuVn100::PublishData(const VnDeviceCompositeData& data) {
  ...
  // const ros::Duration timeSyncIn = ros::Duration().fromSec(data.timeSyncIn/ k1e9);
  // std::cout<<time_since_sync_in<<std::endl;
